"""Integration tests for TaraAgent, using MockLLMClient (ADR-011)."""
...
def request_() -> AgentRequest:
    return AgentRequest(session_id="sess-tara-1", request_id="req-1", user_query="What's our FX and risk exposure?")


def _agent(fixture_name: str, snapshot, tmp_path) -> TaraAgent:
    llm = MockLLMClient.from_fixture(FIXTURES / fixture_name)
    audit_logger = AuditLogger("sess-tara-1", audit_dir=tmp_path)
    return TaraAgent(llm_client=llm, snapshot=snapshot, audit_logger=audit_logger)


def test_happy_path_populates_response_and_writes_audit_log(snapshot, request_, tmp_path):
    agent = _agent("tara_happy_path.json", snapshot, tmp_path)
    response = agent.run(request_)

    assert response.status == ResponseStatus.COMPLETE
    assert response.agent_id == "TARA"
    assert len(response.fx_exposures) > 0
    assert response.var_metrics.var_1d > 0
    assert response.risk_summary.total_fx_exposure > 0
    assert response.recommendations == []

    entries = agent.audit_logger.read_session("sess-tara-1")
...
def test_error_path_returns_error_status_and_logs_failure(snapshot, request_, tmp_path):
    agent = _agent("tara_error_path.json", snapshot, tmp_path)
    response = agent.run(request_)

    assert response.status == ResponseStatus.ERROR
    assert "not be negative" in response.reasoning


def test_approval_gate_sets_pending_approval_status(snapshot, request_, tmp_path):
    agent = _agent("tara_approval_gate.json", snapshot, tmp_path)
    response = agent.run(request_)
